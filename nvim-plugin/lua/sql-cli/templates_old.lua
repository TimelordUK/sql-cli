...
ORDER BY TradeDate DESC]],
    variables = {
      ...
      API_TOKEN = { prompt = "API Token", default = "@{VAR:JWT_TOKEN}", env = true },
...
WITH WEB trades AS (
    ...
    BODY '{
        "select": "*",
        "where": "Source = @{JVAR:SOURCE} and TradeDate = @{DATE:Trade Date}"
    }'
    FORMAT JSON
    JSON_PATH 'Result'
)
...
function M.substitute_variables(template_str, variables)
  ...
  -- DEPRECATED: Old ${VAR} patterns - convert to @{VAR:name} for consistency
  -- This is a fallback only, all templates should use @{VAR:name} syntax
  result = result:gsub("${([^}:]+):?([^}]*)}", function(var, default)
    debug_log("WARNING: Found deprecated ${VAR} syntax for: " .. var .. ". Use @{VAR:" .. var .. "} instead.")
    ...
    elseif default and default ~= "" then
      ...
      return "@{VAR:" .. var .. "}"  -- Convert to new syntax
    end
  end)

  -- Handle remaining environment variables (legacy fallback)
...
function M.expand_functions_in_string(text, callback, test_values, depth)
  depth = depth or 0
  local max_depth = 10

  debug_log("expand_functions_in_string called with text length: " .. #text .. ", depth: " .. depth)

  -- Infinite loop protection
  if depth >= max_depth then
    debug_log("ERROR: Maximum recursion depth (" .. max_depth .. ") reached. Possible infinite loop in macro expansion.")
    vim.notify("Macro expansion error: Maximum recursion depth reached. Check for circular macro references.", vim.log.levels.ERROR)
    callback(nil)
    return
  end
  ...
  M.resolve_functions(functions_to_eval, test_values, function(resolved_values)
    ...
    if result:match("@{[^}]+}") then
      ...
      -- Recursively expand any new patterns with depth tracking
      M.expand_functions_in_string(result, callback, nil, depth + 1)
    else
...
function M.collect_function_patterns(text)
  ...
  -- Updated pattern matching for @{TYPE:args} with proper quote handling
  -- This handles: @{INPUT:prompt}, @{PICKER:macro:prompt}, @{DATE:prompt}, @{VAR:name}, @{JVAR:name}, etc.
  local temp_text = text

  -- First pass: handle patterns with escaped quotes
  while true do
    local match_start, match_end, prefix, func_type, args, suffix = temp_text:find('(\\"?)@{([^:}]+):([^}]+)}(\\"?)')
    if not match_start then break end

    local match = temp_text:sub(match_start, match_end)
    local placeholder = "__FUNC_" .. function_index .. "__"

    local has_escaped_quotes = prefix == '\\"' and suffix == '\\"'
    local has_quotes = prefix == '"' and suffix == '"' and not has_escaped_quotes
    ...
    for part in args:gmatch("[^:]+") do
      table.insert(arg_parts, part:match("^%s*(.-)%s*$")) -- trim whitespace
    end
    ...
    -- Replace with placeholder, preserving quotes exactly
    local replacement
    ...
    expanded_text = expanded_text:gsub(match:gsub("([%(%)%.%[%]%+%-%*%?%^%$%%{}\\])", "%%%1"), replacement, 1)

    -- Continue searching from after the current match
    temp_text = temp_text:sub(match_end + 1)
    function_index = function_index + 1
  end

  -- Second pass: handle patterns without quotes (direct @{TYPE:args})
  temp_text = expanded_text
  while true do
    local match_start, match_end, func_type, args = temp_text:find('@{([^:}]+):([^}]+)}')
    if not match_start then break end

    local match = temp_text:sub(match_start, match_end)
    local placeholder = "__FUNC_" .. function_index .. "__"

    debug_log("Found unquoted @{} function: " .. func_type .. ":" .. args .. " -> " .. placeholder)
    ...
    expanded_text = expanded_text:gsub(match:gsub("([%(%)%.%[%]%+%-%*%?%^%$%%{}\\])", "%%%1"), placeholder, 1)
  ...
  -- Legacy support: Find old function patterns (deprecated)
  ...
  for quoted_func in expanded_text:gmatch('\\"([%w_]+%b())\\"') do
    ...
    if func_name then
      local placeholder = "__FUNC_" .. function_index .. "__"
      debug_log("Found escaped quoted function (DEPRECATED): " .. func_name .. " -> " .. placeholder)
      table.insert(functions_to_eval, {
        pattern = '\\"' .. quoted_func .. '\\"',
        name = func_name,
        args = args,
  ...
  -- Then regular quoted function calls like "Input(...)" (deprecated)
  for quoted_func in expanded_text:gmatch('"([%w_]+%b())"') do
    local func_name, args = quoted_func:match("([%w_]+)%((.*)%)")
    if func_name then
      local placeholder = "__FUNC_" .. function_index .. "__"
      debug_log("Found quoted function (DEPRECATED): " .. func_name .. " -> " .. placeholder)
      table.insert(functions_to_eval, {
        pattern = '"' .. quoted_func .. '"',
        name = func_name,
        args = args,
  ...
  -- Then unquoted function calls (deprecated, only specific known functions)
  ...
  for func_call in expanded_text:gmatch("([%w_]+%b())") do
    ...
    if func_name and known_functions[func_name] then
      local placeholder = "__FUNC_" .. function_index .. "__"
      debug_log("Found unquoted function (DEPRECATED): " .. func_name .. " -> " .. placeholder)
      table.insert(functions_to_eval, {
        pattern = func_call,
        name = func_name,
        args = args,
...
function M.resolve_functions(functions_to_eval, test_values, callback)
  ...
  local func_idx = 1
  local function_cache = {}  -- Cache for deduplicating function calls
  ...
  local function evaluate_next_func()
    ...
    -- Check function cache for duplicate calls (to prevent duplicate prompts)
    local cache_key = func_def.name .. ":" .. (func_def.args or "")
    if function_cache[cache_key] then
      debug_log("Using cached value for " .. cache_key)
      resolved_values[func_idx - 1] = function_cache[cache_key]
      evaluate_next_func()
      return
    end
      ...
      if var_name then
        ...
        if file_macros._CONFIG and file_macros._CONFIG[var_name] then
          ...
          if value and type(value) == "string" then
            value = value:gsub("${([^}]+)}", function(env_var_name)
              -- Check if this is the token variable and use fresh token
              if env_var_name == (token_manager and token_manager.config and token_manager.config.token_var_name) then
                local token = token_manager and token_manager.get_token()
                if token then
                  debug_log("  Resolved token variable: " .. env_var_name)
                  return token
                end
              end
              ...
              if env_value then
                debug_log("  Resolved env var: " .. env_var_name .. " = " .. env_value)
                return env_value
              else
                debug_log("  WARNING: Env var not found: " .. env_var_name)
                return "${" .. env_var_name .. "}"
              end
            end)
          end
          ...
          value = os.getenv(var_name) or vim.env[var_name]
          if value then
            debug_log("  Found in ENV: " .. value)
          end
        end

        local result = value or ("@{VAR:" .. var_name .. "}")
        resolved_values[func_idx - 1] = result
        function_cache[cache_key] = result  -- Cache the result
        evaluate_next_func()
      else
    ...
    elseif func_def.name == "JVAR" then
      ...
      if var_name then
        ...
        local result = value and ('\\"' .. value .. '\\"') or ('\\"@{JVAR:' .. var_name .. '}\\"')
        resolved_values[func_idx - 1] = result
        function_cache[cache_key] = result  -- Cache the result
        evaluate_next_func()
      else
    ...
    elseif func_def.name == "INPUT" or func_def.name == "Input" then
      ...
      vim.ui.input({
        ...
        if value ~= nil then
          -- Return raw value
          resolved_values[func_idx - 1] = value
          function_cache[cache_key] = value  -- Cache the result
          evaluate_next_func()
        else
          callback(nil)
        end
      ...
      if #choices > 0 then
        vim.ui.select(choices, {
          ...
          if choice then
            ...
            function_cache[cache_key] = choice  -- Cache the result
            evaluate_next_func()
          else
            callback(nil)
          end
        ...
        vim.ui.input({
          ...
          if value ~= nil then
            ...
            function_cache[cache_key] = formatted_value  -- Cache the result
            evaluate_next_func()
          else
            -- User cancelled
            callback(nil)
...
function M.test_macro_expansion(macro_text, test_values)
  ...
  if test_values then
    ...
    for i, func_def in ipairs(collection_result.functions) do
      ...
      elseif test_values[func_def.name] then
        ...
        -- Try to match by full function signature (e.g., "VAR:BASE_URL")
        local full_key = func_def.name .. ":" .. (func_def.args or "")
        if test_values[full_key] then
          test_map[func_def.placeholder] = test_values[full_key]
        end
      end
    end
  end
  ...
  for i, func_def in ipairs(collection_result.functions) do
    if test_map[func_def.placeholder] then
      ...
      if func_def.name == "INPUT" or func_def.name == "Input" then
        resolved_values[i] = "TEST_INPUT"
      elseif func_def.name == "JINPUT" then
        resolved_values[i] = '\\"TEST_INPUT\\"'
      elseif func_def.name == "PICKER" or func_def.name == "Picker" then
        resolved_values[i] = "TEST_CHOICE"
      elseif func_def.name == "JPICKER" then
        resolved_values[i] = '\\"TEST_CHOICE\\"'
      elseif func_def.name == "VAR" then
        resolved_values[i] = "TEST_VAR_VALUE"
      elseif func_def.name == "JVAR" then
        resolved_values[i] = '\\"TEST_VAR_VALUE\\"'
      elseif func_def.name == "DATE" or func_def.name == "DateTimePicker" or func_def.name == "DateTime" then
        resolved_values[i] = "DateTime(2025, 1, 1)"
      elseif func_def.name == "MACRO" then
        resolved_values[i] = "EXPANDED_MACRO_CONTENT"
      else
...
WITH WEB trades AS (
    ...
    WHERE_SOURCE = 'Source = @{JVAR:SOURCE}',
    WHERE_DATE = 'TradeDate = @{DATE:Trade Date}',
