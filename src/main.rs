...
use sql_cli::data::csv_datasource;
use sql_cli::refactoring;
...
fn main() -> io::Result<()> {
    ...
    // Check for distinct column analysis
    if let Some(pos) = args.iter().position(|arg| arg == "--distinct-column") {
        if let Some(column_name) = args.get(pos + 1) {
            // Get the query from -q or --query
            let query_pos = args.iter().position(|arg| arg == "-q" || arg == "--query");
            if let Some(qpos) = query_pos {
                if let Some(query) = args.get(qpos + 1) {
                    // Check if query already contains a CTE (WITH keyword)
                    let distinct_query = if query.trim().to_uppercase().starts_with("WITH ") {
                        // Extract CTE name and build distinct query
                        // Look for pattern: WITH [WEB] <name> AS (...)
                        let cte_name = if let Some(pos) = query.find(" AS ") {
                            ...
                            let name_part = before_as
                                ...
                                .trim_start_matches("web ")
                                .trim();
                            name_part.split_whitespace().next().unwrap_or("data")
                        } else {
                            "data"
                        };

                        // Extract just the CTE part (everything up to and including the closing paren)
                        // Find where the CTE AS clause closes
                        ...
                        let cte_part = &query[..cte_end_pos];
                    ...
                    match sql_cli::non_interactive::execute_non_interactive(config) {
                        Ok(_) => {
                            // Success - output already written to stdout in CSV format
                        }
                        Err(e) => {
...
fn handle_banding_generation(args: &[String]) -> io::Result<()> {
...
fn handle_case_range_generation(args: &[String]) -> io::Result<()> {
    use refactoring::banding::CaseGenerator;
