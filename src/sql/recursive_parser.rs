...
// Cursor context now lives with the token-driven analyzer that produces it
// (T9). Re-exported so `use recursive_parser::CursorContext` keeps working.
pub use crate::sql::cursor_context::CursorContext;

/// Where the cursor is, for completion.
///
/// Delegates to [`crate::sql::cursor_context::analyze`]. Before T9 this
/// branched on whether the partial query happened to parse and ran one of two
/// near-identical string scanners; the AST it went to the trouble of building
/// was consulted only for `stmt.where_clause.is_some()`-shaped questions the
/// token stream answers directly. The parse went with them, so this is also
/// one less full parse of the query per keystroke.
#[must_use]
pub fn detect_cursor_context(query: &str, cursor_pos: usize) -> (CursorContext, Option<String>) {
    crate::sql::cursor_context::analyze(query, cursor_pos)
}
