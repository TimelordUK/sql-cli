use sql_cli::parser::{SqlParser, Schema, ParseState};
...
fn main() {
    ...
    schema.set_single_table("trade_deal".to_string(), vec![
        "id".to_string(),
        "price".to_string(), 
...
fn test_sql_parser(schema: Schema) {
    ...
    let context = parser.get_completion_context("SELECT counterparty, quantity FROM trade_deal WHERE status = \"active\" ORDER BY ");
    let suggestions = context.get_suggestions(&schema);
    println!("   Selected columns: {:?}", context.selected_columns);
    println!("   Suggestions: {:?}", suggestions);
}
