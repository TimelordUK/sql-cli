...
fn main() -> anyhow::Result<()> {
    ...
    // Load the CSV file
    csv_client.load_csv("/home/me/dev/sql-cli/BusinessCrimeBoroughLevel.csv", "crime_data")?;
                ...
                if result.count > 0 {
                    ...
                    for row in result.data.iter().take(100) {
                        if let Some(obj) = row.as_object() {
                            ...
                            if let Some(desc) = obj.get("Major Class Description").and_then(|v| v.as_str()) {
                                unique_descriptions.insert(desc);
                            }
                        }
                    }
                    
                    println!("Sample Boroughs: {:?}", unique_boroughs.iter().take(5).collect::<Vec<_>>());
                    println!("Sample Crime Types: {:?}", unique_descriptions.iter().take(3).collect::<Vec<_>>());
                }
            }
            Err(e) => {
                println!("Error: {}", e);
