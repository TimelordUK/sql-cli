...
fn main() -> anyhow::Result<()> {
    ...
    let cache_id1 = cache.save_query(query1, &sample_data, Some("January 2024 trades".to_string()))
    ...
    let cache_id2 = cache.save_query(query2, &bank_data, Some("Bank trades".to_string()))
    ...
    let (loaded_query, loaded_data) = cache.load_query(cache_id1)
    ...
    let result = csv_client.query_csv("SELECT * FROM cached_data WHERE instrumentName.StartsWith('A')")?;
    println!("  instrumentName.StartsWith('A'): {} rows", result.count);
    
    let result = csv_client.query_csv("SELECT * FROM cached_data WHERE counterparty.Length() > 10")?;
