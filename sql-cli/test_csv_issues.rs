...
use sql_cli::parser::{SqlParser, Schema};
...
fn test_table_with_hyphen() -> Result<()> {
    ...
    match client.query_csv("SELECT * FROM \"customers-10000\" LIMIT 1") {
        Ok(result) => println!("✓ Query with quoted table name works: {} rows", result.data.len()),
    ...
    parser.update_single_table("customers-10000".to_string(), vec![
...
fn test_columns_with_spaces() -> Result<()> {
    ...
    match client.query_csv("SELECT \"Customer ID\", \"Phone 1\" FROM customers ORDER BY \"Phone 1\"") {
        Ok(result) => {
            println!("✓ ORDER BY with quoted column works: {} rows", result.data.len());
...
fn test_case_sensitivity() -> Result<()> {
    ...
    match client.query_csv("SELECT city FROM customers") {
        Ok(result) => println!("✗ Lowercase 'city' incorrectly accepted: {} rows", result.data.len()),
    ...
    match client.query_csv("SELECT City FROM customers ORDER BY City") {
        Ok(result) => {
            println!("✓ ORDER BY with correct case works: {} rows", result.data.len());
            for (i, row) in result.data.iter().take(3).enumerate() {
                if let Some(obj) = row.as_object() {
    ...
    let mut parser = CursorAwareParser::new();
    parser.update_single_table("customers".to_string(), vec![
        "Customer ID".to_string(),
        "First Name".to_string(),
        "City".to_string(),
        "Phone 1".to_string(),
