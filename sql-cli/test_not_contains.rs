...
fn main() {
    ...
    client.load_from_json(test_data.clone(), "countries").unwrap();
    
    // Test 1: Countries that contain "Br"
    println!("Test 1 - Countries containing 'Br':");
    let result = client.query_csv(r#"SELECT * FROM countries WHERE Country.Contains("Br")"#).unwrap();
    ...
    let result2 = client.query_csv(r#"SELECT * FROM countries WHERE NOT Country.Contains("Br")"#).unwrap();
