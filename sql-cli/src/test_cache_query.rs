...
mod tests {
    ...
    fn test_cache_mode_query_filtering() {
        ...
        csv_client.load_from_json(test_data.clone(), "cached_data").unwrap();
        
        let result = csv_client.query_csv("SELECT * FROM cached_data WHERE commission > 50").unwrap();
        assert_eq!(result.data.len(), 2, "Should return 2 rows with commission > 50");
        
        // Test 2: Contains filter
        let result = csv_client.query_csv("SELECT * FROM cached_data WHERE counterparty.Contains(\"Bank\")").unwrap();
        assert_eq!(result.data.len(), 3, "Should return 3 rows with 'Bank' in counterparty");
        
        // Test 3: Combined filters
        let result = csv_client.query_csv("SELECT * FROM cached_data WHERE commission > 50 AND counterparty.Contains(\"Bank\")").unwrap();
        assert_eq!(result.data.len(), 2, "Should return 2 rows matching both conditions");
        
        // Test 4: IN clause
        let result = csv_client.query_csv("SELECT * FROM cached_data WHERE counterpartyCountry IN (\"US\", \"DE\")").unwrap();
        assert_eq!(result.data.len(), 3, "Should return 3 rows with countries US or DE");
        ...
        assert_eq!(result.data.len(), 2, "Should return 2 rows matching all conditions");
    }
    ...
    fn test_user_scenario_with_trade_deal() {
        ...
        let mut csv_client = CsvApiClient::new();
        csv_client.load_from_json(test_data.clone(), "trade_deal").unwrap();
        
        // Test the exact query the user mentioned
        let query = "select * from trade_deal where commission > 50 and counterparty.Contains(\"Bank\")";
    ...
    fn test_complex_query_with_in_clause() {
        ...
        assert_eq!(result.data.len(), 4, "Should return 4 rows matching all conditions");
        
        // Verify the countries in results
        for row in &result.data {
            let country = row["counterpartyCountry"].as_str().unwrap();
            assert!(country == "JP" || country == "FR", "Country should be JP or FR, got {}", country);
            
            let commission = row["commission"].as_f64().unwrap();
            assert!(commission > 50.0, "Commission should be > 50, got {}", commission);
            
            let counterparty = row["counterparty"].as_str().unwrap();
            assert!(counterparty.contains("Bank"), "Counterparty should contain 'Bank', got {}", counterparty);
        }
    }
}