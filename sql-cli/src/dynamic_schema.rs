...
impl SchemaManager {
    ...
    pub fn load_schema(&mut self) -> Result<HashMap<String, TableSchema>, Box<dyn std::error::Error>> {
    ...
    fn fetch_from_server(&self) -> Result<HashMap<String, TableSchema>, Box<dyn std::error::Error>> {
        // For now, return error to trigger fallback
        // TODO: Implement actual API call when server endpoint is ready
        Err("Schema API not yet implemented".into())
    }
    
    fn load_from_cache(&mut self) -> Result<HashMap<String, TableSchema>, Box<dyn std::error::Error>> {
    ...
    fn save_cache(&self, schemas: &HashMap<String, TableSchema>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn get_default_schema(&self) -> HashMap<String, TableSchema> {
        ...
        for table_config in config.tables {
            ...
            methods.insert("datetime".to_string(), vec![
                "DateTime".to_string(),
            ]);
    ...
    fn infer_type(&self, column_name: &str) -> String {
        ...
        if column_name.ends_with("Date") || column_name.ends_with("Time") {
            ...
            "string".to_string()
        } else if column_name == "price" || column_name == "quantity" || 
                  column_name == "commission" || column_name.ends_with("Amount") {
            "decimal".to_string()
        } else {
            "string".to_string()
        }
