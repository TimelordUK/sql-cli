...
impl<'a> StatefulWidget for VirtualTable<'a> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        // Create header
        let header_cells: Vec<ratatui::widgets::Cell> = self.headers
            .iter()
        ...
        let rows: Vec<Row> = visible_slice.iter().enumerate().map(|(idx, record)| {
            let absolute_idx = state.offset + idx;
            let is_selected = absolute_idx == state.selected;
            
            let cells: Vec<ratatui::widgets::Cell> = self.headers.iter().map(|field| {
