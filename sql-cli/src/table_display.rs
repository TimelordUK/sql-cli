...
pub fn display_results(data: &[Value], fields: &[String]) {
    ...
    let headers: Vec<Cell> = if fields.contains(&"*".to_string()) {
        ...
        if let Some(first) = data.first() {
            if let Some(obj) = first.as_object() {
                obj.keys().map(|k| Cell::new(k).add_attribute(Attribute::Bold)).collect()
            } else {
                vec![]
            }
        } else {
        ...
        fields.iter().map(|f| Cell::new(f).add_attribute(Attribute::Bold)).collect()
    };
    ...
    for record in data {
        if let Some(obj) = record.as_object() {
            let row: Vec<String> = field_names.iter()
                .map(|field| {
                    match obj.get(field) {
...
pub fn export_to_csv(data: &[Value], fields: &[String], filename: &str) -> Result<(), Box<dyn std::error::Error>> {
