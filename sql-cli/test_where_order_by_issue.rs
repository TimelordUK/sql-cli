...
fn test_where_with_order_by() {
    ...
    client.load_from_json(test_data.clone(), "products").unwrap();
    
    let result = client.query_csv(r#"SELECT * FROM products WHERE Availability NOT IN ("pre_order")"#).unwrap();
    ...
    client2.load_from_json(test_data.clone(), "products").unwrap();
    
    let result2 = client2.query_csv(r#"SELECT * FROM products ORDER BY Availability"#).unwrap();
    ...
    println!("  First: {}, Last: {}", first_availability, last_availability);
    ...
    client3.load_from_json(test_data.clone(), "products").unwrap();
    
    let query = r#"SELECT * FROM products WHERE Availability NOT IN ("pre_order") ORDER BY Availability"#;
    ...
    let availabilities: Vec<String> = result3.data.iter()
    ...
    for i in 1..availabilities.len() {
        assert!(availabilities[i-1] <= availabilities[i], 
            "Results not ordered: {} > {}", availabilities[i-1], availabilities[i]);
    }
    
    // Test 4: ORDER BY with price to verify numeric sorting works with WHERE
    let mut client4 = CsvApiClient::new();
    client4.load_from_json(test_data.clone(), "products").unwrap();
    
    let result4 = client4.query_csv(r#"SELECT * FROM products WHERE Availability NOT IN ("pre_order") ORDER BY Price"#).unwrap();
    let prices: Vec<f64> = result4.data.iter()
    ...
    for i in 1..prices.len() {
        assert!(prices[i-1] <= prices[i], 
            "Prices not ordered: {} > {}", prices[i-1], prices[i]);
    }
}
