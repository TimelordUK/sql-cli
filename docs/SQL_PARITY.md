...
- **Status:** 🟢 FIXED 2026-09-13 as [R13](ENGINE_REFACTORING.md#r13) slice 2 —
  168 → 174 AGREE with [P50](#p50). All six faults below closed in one change.
- **Corpus:** `01_select.toml :: select_col_vs_col_null_operand`,
  `select_eq_null_literal`, `select_in_list_with_null`, `select_and_with_null`,
  `simple_case_when_null` (all were DIFFER, now AGREE). Per operator:
  `tests/evaluator_matrix_tests.rs`, whose value-evaluator column went from 4/36
  to 36/36.
...
- **Fix (2026-09-13).** `ArithmeticEvaluator` now evaluates predicates in
  three-valued logic with NULL standing for UNKNOWN. Comparisons go through a
  predicate-layer `compare_trilean` (NULL on either side → NULL) — the shared
  `compare_values` still reports `NULL = NULL` as equal, because ORDER BY needs
  it, exactly as R10 left it. `AND`/`OR`/`NOT` use `Trilean`'s truth tables;
  `to_bool` is gone. `IN`/`NOT IN` take OR's table, `BETWEEN` AND's, `LIKE`
  yields NULL. `Trilean::from_value`/`to_value` are the one crossing between the
  two representations. Found at the start of the slice and pinned first: simple
  `CASE x WHEN NULL` matched a NULL `x` through a separate `values_equal`, now
  also fixed.
- **What did not move, deliberately:** join matching ([P52](#p52), slice 6) and
  case-insensitive comparison in this evaluator (slice 3). FORMAL examples had
  no expectation churn — none captured a projected `false` for NULL.
---
...
- **Status:** 🟢 FIXED 2026-09-13 by [R13](ENGINE_REFACTORING.md#r13) slice 2, with
  [P48](#p48). The comparison now reaches `NOT` as NULL, so it stays NULL and
  `is_truthy` drops the group. `is_truthy` itself remains, to be replaced by the
  single collapse in slice 6.
- **Corpus:** `07_grouping.toml :: having_not_over_null` (was DIFFER, now AGREE).
