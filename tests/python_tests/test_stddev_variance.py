...
50"""
    ...
    def test_variance_simple(self, numeric_data):
        ...
        assert abs(variance - 200.0) < 0.001
    ...
    def test_stddev_simple(self, numeric_data):
        ...
        assert abs(stddev - math.sqrt(200)) < 0.001
...
50"""
    ...
    def test_variance_ignores_nulls(self, data_with_nulls):
        ...
        # Values: 10, 30, 50
        # Mean: 30
        # Variance: ((10-30)² + (30-30)² + (50-30)²) / 3 = (400 + 0 + 400) / 3 ≈ 266.67
        assert abs(variance - 266.666666) < 0.001
    ...
    def test_stddev_ignores_nulls(self, data_with_nulls):
        ...
        assert len(results) == 1
        stddev = float(results[0]['stddev'])
        # StdDev = sqrt(266.67) ≈ 16.33
        assert abs(stddev - math.sqrt(266.666666)) < 0.001
...
C,100"""
    ...
    def test_stddev_with_group_by(self, grouped_data):
        ...
        # StdDev for A and B should be sqrt(66.67) ≈ 8.165
        assert abs(float(results[0]['stddev']) - math.sqrt(66.666666)) < 0.001
        assert abs(float(results[1]['stddev']) - math.sqrt(66.666666)) < 0.001
        # StdDev for C should be 0 (single value)
        assert abs(float(results[2]['stddev']) - 0.0) < 0.001
    ...
    def test_variance_with_having(self, grouped_data):
        ...
        # Should only return A and B (C has variance=0)
...
10"""
    ...
    def test_stddev_single_value(self, single_value):
        """Test STDDEV with single value (should be 0)."""
        query = "SELECT STDDEV(value) as stddev FROM test"
        results = run_sql_query(single_value, query)
...
50"""
    ...
    def test_variance_mixed_types(self, mixed_numeric_data):
        ...
        variance = float(results[0]['variance'])
        # Mean = 30.2
        # Variance calculation with mixed types
        mean = 30.2
